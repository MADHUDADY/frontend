...
export default function KioskPage() {
    ...
    try {
      ...
      try {
        const res = await axios.post(`${QMS_API}/ticket/newticketdoctor`, {
          clinicId: CLINIC_ID, serviceId: Number(doc.doctorid),
          ticketType: type, zone: 1,
          appointmentNo: appt?.AppointNumber || "",
          patientName:   appt?.PatientName   || "",
        });
        num = res.data?.ticketnumber || res.data?.ticketNumber || "";
      } catch {}
      if (!num) {
  ...
  const searchAppointments = async () => {
    ...
    try {
      ...
      setScreen("appt_list");
    } catch { setAppointments([]); setScreen("appt_list"); }
    finally { setBusy(false); }
  };
...
const S: Record<string, React.CSSProperties> = {
  ...
  bar: {
    background: "linear-gradient(135deg,#0d1b2a 0%,#1a3a5c 60%,#0f2c45 100%)",
    color: "#fff",
    display: "flex",
    alignItems: "center",
  ...
  dstats: {
    display: "flex", gap: 12, fontSize: 12, color: "#555", fontWeight: 500,
  ...
  card: {
    ...
    boxShadow: "0 4px 32px rgba(15,32,60,0.10)",
    border: "1px solid #e8edf5",
  ...
  ib: {
    ...
    padding: "4px 18px", width: "100%", margin: "16px 0 24px",
